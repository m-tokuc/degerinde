...
INPUT_FILE = "oracle_links.txt"
OUTPUT_FILE = "oracle_araba_verileri.jsonl"
CHECKPOINT_FILE = "oracle_islenen_linkler.txt"
CONCURRENCY_LIMIT = 3  # Anti-Ban Sweet Spot (Zero 429s)
MAX_RETRIES = 5

# --- LOGGING SETUP ---
logger = logging.getLogger('DetayCekiciOracle')
...
def parse_html(html, url):
    ...
    try:
        ...
        details["Tramer"] = trramer_text if 'trramer_text' in locals() else tramer_text
...
async def main():
    if not os.path.exists(INPUT_FILE):
        logger.error(f"'{INPUT_FILE}' bulunamadı!")
        return

    processed_set = load_processed_links()
    
    logger.info(f"'{INPUT_FILE}' dosyasından linkler okunuyor...")
    ...
    logger.info("👻 Oracle Hayalet Motoru Başlatılıyor...")
    ...
    logger.info("🎉 İşlem Tamamlandı! Tüm veriler oracle_araba_verileri.jsonl içine kaydedildi.")
