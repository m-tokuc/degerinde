...
if not MODEL_PATH:
    if os.path.exists("models/v2/model.pkl"):
...
def dynamic_options(req: DynamicOptionsRequest):
    ...
    # 5. Diğer özellikler (Vites, Yakıt, Renk vb.) kullanıcıyı KISITLAMAMAK için 
    # sadece seçili "Marka" (df_seri) bazında döndürülür, "Seri/Model/Yıl"a göre daraltılmaz.
    # Eğer Marka da seçilmemişse tüm global listeyi (df) döndürür.
    df_genis = df_seri if not df_seri.empty else df
    ...
    return {
        ...
        "modeller": modeller,
        "yillar": yillar_list,
        "vitesler": unique_sorted(df_genis["Vites_Tipi"]) if "Vites_Tipi" in df_genis else ["Manuel", "Otomatik", "Yarı Otomatik"],
        "yakitlar": unique_sorted(df_genis["Yakit_Tipi"]) if "Yakit_Tipi" in df_genis else ["Benzin", "Dizel", "LPG", "Elektrik", "Hibrit"],
        "kasalar": unique_sorted(df_genis["Kasa_Tipi"]) if "Kasa_Tipi" in df_genis else ["Sedan", "Hatchback", "SUV", "Station Wagon"],
        "renkler": unique_sorted(df_genis["Renk"]) if "Renk" in df_genis else ["Beyaz", "Siyah", "Gri", "Gümüş", "Kırmızı", "Mavi"],
        "kimden": unique_sorted(df_genis["Kimden"]) if "Kimden" in df_genis else ["Sahibinden", "Galeriden"],
        # Frontend'de kaybolan Boolean/Statik özellikler her halükarda garantileniyor:
        "garanti_durumu": ["Garantisi Var", "Garantisi Yok", "Belirtilmemiş"],
        "yedek_anahtar": ["Yedek Anahtarı Var", "Yedek Anahtarı Yok", "Belirtilmemiş"],
    }
...
def predict(req: CarFeaturesRequest, request: Request):
    ...
    # SHAP Analizi (Yapay Zeka Açıklanabilirliği) - MATEMATİKSEL DİKTATÖRLÜK
    fiyat_etkenleri = []
    if 'explainer' in globals() and explainer and explainer.is_ready:
        try:
            ...
            if "error" not in shap_result:
                hasar_var = hasar["Has_Boya"] or hasar["Has_Degisen"] or hasar["Tramer_TL"] > 0
                
                if hasar_var and shap_result.get("damage_impact", 0) > 0:
                    # Transfer illogical positive damage impact to other factors to preserve mathematical sum
                    shap_result["other_impact"] = shap_result.get("other_impact", 0) + shap_result["damage_impact"]
                    shap_result["damage_impact"] = 0
                ...
                for key, val in shap_result.items():
                    if key in mapping and abs(val) > 100:
                        fiyat_etkenleri.append({
                            "isim": mapping[key],
                            "miktar": int(val),  # abs() KALDIRILDI! FRONTEND'E GERÇEK DEĞER GİDİYOR
                            "yon": "pozitif" if val > 0 else "negatif"
                        })
                # Etki miktarına (mutlak değere) göre en büyükten küçüğe sırala
                fiyat_etkenleri.sort(key=lambda x: abs(x["miktar"]), reverse=True)
        except Exception as e:
    ...
    return {
        "tahmini_fiyat": fiyat,
        "base_fiyat": int(shap_result.get("base_price", fiyat)) if 'shap_result' in locals() and "error" not in shap_result else fiyat,
        "fiyat_araligi": {
            "min": min_fiyat,
            "max": max_fiyat
        },
